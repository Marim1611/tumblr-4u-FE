    ...
    >
      ...
      <div class="footer-flex dropdown">
        ...
        >
          ...
          <div class="clickable reactionsWind border-bottom v-flex text-left">
            <div class="h-flex no-padding">
              <img
                ...
                :key="i"
                class="img-reactions"
                src="https://64.media.tumblr.com/51b7d5e9cfaa579ec94a17e618d96a1a/95b6dc889657dcc9-3c/s64x64u_c1/283ac5792129caa8fa116b4560861ff3431d7e3c.pnj"
              />
            </div>
          ...
          <div class="border-bottom grow v-flex comments">
            <div
              id="comments"
              class="no padding"
              v-for="(comment, i) in commentsComputed"
              :key="i"
            >
              <div class="h-flex">
        ...
        >
          ...
          <div class="h-flex message-result">
            <!-- if want to put a line    <div class=" border-bottom message-result">-->
          </div>
          <!--other sharing options(to out of tumblr like: facebook, twitter...etc)-->
          <div class="out-share h-flex">
            <div
              v-clipboard:copy=""
              v-on:click="copyShare"
              class="share-item clickable"
            >
              <b-icon
                class="img-icon"
                icon="files"
                ...
                aria-hidden="true"
              ></b-icon>
              <p v-bind:style="{ color: homeTheme[homeThemeIndex].fontColor }">
                {{ shareCopy }}
              </p>
            </div>
        ...
        <ReblogWindow
          v-on:reblogWind="reblogWind($event)"
          v-show="reblog"
          v-bind:postName="postName"
...
export default {
  ...
  async created() {
    ...
    try {
      ...
      else myRoute = Browser().baseURL + `/${this.post.notesId}/notes`;
      await axios
        .get(myRoute, {
    ...
    try {
      ...
      else myRoute2 = Browser().baseURL + `/blog/view/${this.post.blogId}`;
      await axios
        .get(myRoute2, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        })
    ...
    async commenting() {
      ...
      try {
        ...
        if (this.isMockServer(Browser().baseURL))
          ...
          myRoute =
            Browser().baseURL +
            `/${this.getPrimaryBlogId}/${this.post._id}/comment`;
        await axios
          .put(
            myRoute,
            {
    ...
    async like(post) {
      try {
        ...
        if (this.isMockServer(Browser().baseURL))
          ...
          myRoute =
            Browser().baseURL +
            `/${this.getPrimaryBlogId}/${post._id}/like_press`;
      ...
      for (let i = 0; i < this.commentsComputed.length; i++) {
        try {
          ...
          if (this.isMockServer(Browser().baseURL))
            ...
            myRoute =
              Browser().baseURL +
              `/blog/view/${this.commentsComputed[i].blogId}`;
          await axios
            .get(myRoute, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
